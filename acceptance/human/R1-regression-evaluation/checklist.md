...
- 所需人工角色：评测负责人
- 清单负责人：用户
- 批准证据：用户于 2026-09-01 批准 R1 验收并要求尽量使用 AI 审阅，随后明确使用 local CI；v11 新增冻结清单完整性、运行来源和路径组件隔离前置条件
...
- 前置条件：机器测试、冻结清单全量校验、R1 evidence 和两份绑定同一冻结输入的独立 AI `PASS` 报告均已通过；打开对应的回归结果与限制说明。
- 验收步骤：
  1. 查看冻结清单是否逐项全量校验通过，并确认两份复审报告具有不同审阅身份、不同包装器和相同冻结输入摘要，且均以 `PASS` 结束。
