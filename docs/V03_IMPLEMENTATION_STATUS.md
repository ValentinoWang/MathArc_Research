...
| Publication technical-preflight fixture | IMPLEMENTED | `make publication-gate` runs a deterministic synthetic workspace/LaTeX/claim-map audit through `audit_publication`; it is included in `quality`, `make ci`, and `make ci-full`. The fixture is explicitly non-authorizing and does not represent a real-paper or publication result. |
