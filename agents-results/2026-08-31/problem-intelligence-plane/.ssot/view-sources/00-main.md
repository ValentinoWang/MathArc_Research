...
**修订 6：调色板改版。**本节的取值在 2026-09-03 由项目负责人决定整体改版，目标是把控制台从「学术论文纸面」调向「仪器台」：中性色由偏暖绿改为偏冷石墨，强调色向青增加纯度，深色模式改为更深的蓝黑机箱色。令牌**名称集合、三态结构、浅色多出的四个非颜色令牌、组件类清单（235 个）与 `@media` 条数（14 条）全部未变**，因此本次修订只重钉 `token_table_sha256`，`class_list_sha256` 与断点映射保持原值。另有两处不改变令牌集合的可访问性修正：强调色与门禁色上的按钮文字由写死的 `#fff` 改为 `var(--on-e)`，使其随主题翻转；深色模式的亮青强调色因此不再承载白字。
...
| 令牌 | 浅色 | 深色（两块相同） | 用途 |
|---|---|---|---|
| `--ground` | `#E9EFF2` | `#060D10` | 页面底色 |
| `--surface` | `#FFFFFF` | `#0D181C` | 卡片与面板底色 |
| `--surface-2` | `#DCE6EA` | `#152329` | 次级底色与代码块 |
| `--ink` | `#0B1618` | `#DCE9EB` | 正文字色 |
| `--muted` | `#516569` | `#7D949B` | 次要说明字色 |
| `--line` | `#C4D2D7` | `#1F333A` | 边框与分隔线 |
| `--accent` | `#00736B` | `#3BD6C4` | 主强调色与焦点环 |
| `--accent-strong` | `#005249` | `#6FE7D8` | 强调色的加重态 |
| `--accent-soft` | `#D6EDE9` | `#0B2E2C` | 强调色的底色 |
| `--ok` | `#12764C` | `#4FCA8B` | 通过态 |
| `--ok-soft` | `#D9EFE3` | `#0C2A1B` | 通过态底色 |
| `--gap` | `#8A5F0A` | `#E0AC4E` | 缺口态 |
| `--gap-soft` | `#F4E9D3` | `#33280F` | 缺口态底色 |
| `--err` | `#B3302B` | `#F0837A` | 错误态 |
| `--err-soft` | `#F7DFDD` | `#3A1B18` | 错误态底色 |
| `--skip` | `#556570` | `#93A3AE` | 不适用态 |
| `--skip-soft` | `#E4E9EC` | `#1E252C` | 不适用态底色 |
| `--hash` | `#3D5B60` | `#82AFAB` | 哈希与摘要字色 |
| `--e1` | `#D5E3E4` | `#1B2A2D` | 证据强度第一级 |
| `--e2` | `#9DC7C2` | `#2F534E` | 证据强度第二级 |
| `--e3` | `#5FAAA1` | `#427A71` | 证据强度第三级 |
| `--e4` | `#248C7E` | `#54A396` | 证据强度第四级 |
| `--e5` | `#0A4A45` | `#7EE0CD` | 证据强度第五级 |
| `--on-e` | `#E8F5F3` | `#060D10` | 证据色上的字色 |
| `--on-e-dim` | `#AED6D0` | `#1B3B37` | 证据色上的弱字色 |
| `--shadow` | `0 1px 2px rgba(11,22,24,.10), 0 6px 20px rgba(11,22,24,.07)` | `0 1px 2px rgba(0,0,0,.55), 0 8px 26px rgba(0,0,0,.42)` | 卡片投影 |
...
若实现者改用继承回填，同一份原型会得到 `cef1c574281e4d2c86628cf6ce517b1c82188f50bf9093579c371a0c875e3f9c`。该值在此登记为**错误规则的判别值**：静态合同一旦算出它，说明用错了填充规则，而不是原型发生了变化。
