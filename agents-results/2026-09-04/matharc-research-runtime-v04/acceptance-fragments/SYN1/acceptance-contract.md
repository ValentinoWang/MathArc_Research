...
The user reaching candidate-synthesis drives item SYN1 (unspecified dimension) through the interface declared for SYN1.

## Problem

Item SYN1 exists because the interface declared for SYN1 does not yet satisfy the acceptance seeds registered for it, leaving candidate-synthesis incomplete.

## Expected outcome

After item SYN1 lands, the interface declared for SYN1 satisfies every acceptance seed below and candidate-synthesis reflects that behavior.

## Non-goals

Item SYN1 covers only the interface declared for SYN1 and candidate-synthesis as described by its acceptance seeds; behavior outside those seeds is out of scope.
...
Then every acceptance seed for item SYN1 holds
```

## Exception paths

If the interface declared for SYN1 fails for item SYN1, candidate-synthesis must surface the failure exactly as the acceptance seeds below specify; no exception handling beyond those seeds is in scope.

## Invariants

For item SYN1, the interface declared for SYN1 must continue to satisfy every acceptance seed below on every call; candidate-synthesis must never show a state the seeds forbid.
...
| --- | --- | --- |
<!-- shared_acceptance_policy: SAP-MATHARC-RUNTIME@1 -->
| none | none | Behavior specification only for item SYN1; executable baseline not yet locked |
