...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | none | machine/integration-contract | 在 matharc/v02/run_store.py 中幂等导入候选、费用和执行回执并保留来源身份 | Integration | Automatic | Yes |
| AC-02 | behavior | none | machine/local-runtime | 在 tests/test_runtime_idempotent_import.py 中证明重复导入结果不变且来源身份变化时拒绝 | Local runtime | Automatic | Yes |
